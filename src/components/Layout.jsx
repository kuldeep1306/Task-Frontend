...
const Layout = ({ user, onLogout }) => {
  ...
  const stats = useMemo(() => {
    ...
    const completionPercentage = totalCount ? Math.round((completedTasks / totalCount) * 100)
        : 0
    return {
      totalCount,
      completedTasks,
    ...
    duration-300 hover:border-purple-100 group">
      <div className="flex items-center gap-2">
        ...
        <div className="min-w-0">
          <p className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-fuchsia-500 to-purple-600 bg-clip-text text-transparent">{value}</p>
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex justify-center items-center">
        <div className="animate-spin h-12 w-12 border-2 border-purple-500 rounded-full " />
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gray-50 p-6 flex justify-center items-center">
        ...
        <button onClick={fetchTasks} className="mt-4 py-2 px-4 bg-red-100 text-red-700 rounded-lg text-sm font-medium hover:bg-red-200
        transition-colors">
          Try Again
        </button>
      </div>
      ...
      <div className="ml-0 xl:ml-64 md:ml-16 pt-16 p-3 sm:p-4 transition-all duration-300">
        ...
        <div className="xl:col-span-1 space-y-4 sm:space-y-6">
          <div className="bg-white rounded-xl sm:p-5 shadow-sm border border-purple-100">
            ...
            <TrendingUp className="w-4 h-4 sm:w-5 sm: text-purple-500" />
            Task Statistics
            </h3>
          

          <div className="grid grid-cols-2 gap-3 mb-4 sm:gap-4 sm:mb-6">
            <StatCard title="Total" value={stats.totalCount} icon={<Circle className="w-3.5 h-3.5 sm:h-4 text-purple-500"/>} />
            <StatCard title="Completed" value={stats.completedTasks} icon={<Check className="w-3.5 h-3.5 sm:h-4 text-purple-500" />} />
            <StatCard title="Pending" value={stats.pendingCount} icon={<Clock className="w-3.5 h-3.5 sm:h-4 text-purple-500 " />} />
            <StatCard title="Completion" value={`${stats.completionPercentage}%`} icon={<Zap className="w-3.5 h-3.5 sm:h-4 text-purple-500" />} />
          </div>
          ...
          <div className="relative pt-2">
            <div className="flex gap-1.5 items-center">
              <div className="flex-1 h-2 sm:h-3 bg-purple-100 rounded-full overflow-hidden">
                ...
                duration-500" style={{ width: `${stats.completionPercentage}%` }} />
              </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100">
            ...
            <div className="space-y-2 sm:space-y-3">
              ...
              >
                {isTaskCompleted(task.completed) ? 'Completed' : 'Pending'}
               ...
               {tasks.length === 0 && (
                <div className="text-center py-4 sm:py-6 px-2">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Task will appear here once you start adding and completing them.
                  </p>
                </div>
